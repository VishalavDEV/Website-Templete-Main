...
export default function Hero({ dateRange, totalSalesCount, grossRevenue }: HeroProps) {
  return (
    <section 
      ...
      className="relative overflow-hidden bg-[#1e293b]/68 backdrop-blur-[18px] border border-[rgba(255, 255, 255, 0.1)] rounded-2xl p-5 sm:p-6 md:p-8 shadow-[0_12px_40px_rgba(63,42,27,0.08)]"
    >
      ...
      <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-5 md:gap-6 z-10">
        <div className="max-w-xl">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-[#f8fafc] tracking-tight leading-tight">
            An easy way to manage sales with <span className="text-[#ff6a3d]">care and precision</span>.
          </h2>
          
          <p className="text-xs sm:text-sm text-[#cbd5e1] mt-2.5 sm:mt-3 leading-relaxed">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full md:w-72 lg:w-80 shrink-0">
          <div className="bg-[#1e293b]/68 backdrop-blur-[18px] border border-[rgba(255, 255, 255, 0.1)] rounded-[10px] p-3.5 sm:p-4 flex flex-col justify-between shadow-xs">
            <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs font-semibold text-[#94a3b8] uppercase tracking-wider">
              <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#ffc94d] shrink-0" />
              <span className="truncate">Period</span>
            </div>
            <div className="mt-3 sm:mt-4">
              <span className="block text-xs font-bold text-[#f8fafc]">Active Range</span>
              <span className="block text-[10px] text-[#cbd5e1] mt-0.5 truncate">{dateRange.startDate} - {dateRange.endDate}</span>
            </div>
          </div>
          ...
          <div className="bg-[#1e293b]/68 backdrop-blur-[18px] border border-[rgba(255, 255, 255, 0.1)] rounded-[10px] p-3.5 sm:p-4 flex flex-col justify-between shadow-xs">
            ...
            <div className="mt-3 sm:mt-4">
              ...
              <span className="block text-[10px] text-[#cbd5e1] mt-0.5 font-medium truncate">
