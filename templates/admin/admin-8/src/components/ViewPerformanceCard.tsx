...
export default function ViewPerformanceCard({ performanceData }: ViewPerformanceCardProps) {
  ...
  return (
    ...
    >
      {/* Card Header */}
      <div className="mb-6">
        <span className="text-xs font-bold text-[#cbd5e1] uppercase tracking-wider">Acquisition Channels</span>
      ...
      <div className="grid grid-cols-3 gap-3 bg-black/[0.02] border border-white/10 rounded-2xl p-4 mb-6">
        <div className="flex flex-col">
          <div className="flex items-center gap-1 text-[10px] font-bold text-[#cbd5e1] uppercase tracking-wider">
            <Eye className="w-3.5 h-3.5 text-[#ff6a3d] shrink-0" />
            <span className="truncate">View Count</span>
          </div>
          <span className="text-sm md:text-base font-extrabold text-[#f8fafc] mt-2 block">
            {aggregates.totalViews.toLocaleString()}
          </span>
          <span className="text-[9px] text-[#94a3b8] mt-0.5">Total Impressions</span>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[9px] text-[#94a3b8] mt-0.5">Click-Through Rate</span>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <span className="text-sm md:text-base font-extrabold text-[#ff6a3d] mt-2 block">
            {aggregates.totalSales} <span className="text-[10px] text-[#cbd5e1] font-semibold">({aggregates.conversionRate.toFixed(1)}%)</span>
          </span>
          <span className="text-[9px] text-[#94a3b8] mt-0.5">Conversion Yield</span>
        </div>
      </div>

      {/* Breakdown bar list */}
      <div className="flex-1 space-y-4">
        <span className="block text-[10px] font-bold text-[#94a3b8] uppercase tracking-wider mb-2">Performance by Stream</span>
        {performanceData.map((item, i) => {
          ...
          return (
            <div key={i} className="flex flex-col gap-1.5 pb-3 border-b border-white/10 last:border-b-0 last:pb-0">
              <div className="flex items-center justify-between text-xs font-semibold text-[#f8fafc]">
                ...
                <span className="text-[#cbd5e1]">{item.views.toLocaleString()} views <span className="text-[10px] font-medium text-[#94a3b8] ml-1">({item.sales} conversions)</span></span>
              </div>
