...
export const PullQuote: React.FC<PullQuoteProps> = ({ quote, attribution }) => {
  return (
    ...
    >
      ...
      <span className="font-serif italic text-6xl sm:text-7xl text-[#ffffff] absolute -top-5 left-0 leading-none select-none">
        “
      </span>
      ...
      {attribution && (
        <figcaption className="mt-6 font-mono text-xs text-[#ffffff] uppercase tracking-widest font-semibold flex items-center gap-2">
          <span className="w-4 h-[1px] bg-[#ffffff]" />
          {attribution}
        </figcaption>
      )}
    </motion.figure>
