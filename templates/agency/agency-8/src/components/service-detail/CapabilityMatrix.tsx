        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {service.capabilityGrid.map((item, index) => (
            <motion.div
              ...
              className="group p-6 sm:p-8 rounded-xl bg-white/[0.02] border border-white/[0.08] hover:border-[#6366f1]/40 hover:bg-white/[0.04] transition-all duration-300 flex flex-col justify-between min-h-[160px] cursor-pointer"
            >
              <div className="flex items-center justify-between">
                <span className="font-mono text-xs text-[#666666] group-hover:text-[#6366f1] transition-colors">
                  0{index + 1}
                </span>
                <Sparkles className="w-3.5 h-3.5 text-white/20 group-hover:text-[#6366f1] transition-colors" />
              </div>

              <div>
                <h3 className="font-display text-lg sm:text-xl font-bold uppercase text-[#FAF9F6] tracking-tight group-hover:text-[#6366f1] transition-colors">
                  {item}
                </h3>
