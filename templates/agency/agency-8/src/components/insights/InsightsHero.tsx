...
export const InsightsHero: React.FC<InsightsHeroProps> = ({
  ...
  return (
    <section className="relative pt-36 sm:pt-44 md:pt-48 pb-20 sm:pb-28 px-6 sm:px-8 lg:px-12 bg-[#080808] border-b border-[#ffffff10] overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-[#ffffff]/[0.035] rounded-full blur-[140px] pointer-events-none" />
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.04] border border-white/10 font-mono text-[11px] uppercase tracking-widest text-[#FAF9F6]">
              <BookOpen className="w-3 h-3 text-[#ffffff]" />
              24 ARTICLES
            </span>
            ...
            {lines.map((line, index) => (
              <div key={line} className="overflow-hidden">
                ...
                >
                  <span>{line}</span>
                  {index === 2 && (
                    <span className="inline-block w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 rounded-full bg-[#ffffff] translate-y-1 sm:translate-y-2" />
                  )}
                </motion.div>
              </div>
            ))}
        ...
        >
          ...
          <div className="md:col-span-4 flex md:justify-end">
            <button
              ...
              className="group inline-flex items-center gap-3 text-xs font-mono uppercase tracking-[0.2em] text-[#FAF9F6] hover:text-[#ffffff] transition-colors cursor-pointer py-2"
            >
              <span>EXPLORE ARTICLES</span>
              <span className="w-8 h-8 rounded-full border border-white/20 group-hover:border-[#ffffff] flex items-center justify-center transition-colors">
